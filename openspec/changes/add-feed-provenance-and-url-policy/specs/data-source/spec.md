...
A feed whose declared integrity verifies SHALL be classified `trusted`. When no integrity is
declared, the feed loads without verification (trust classification still applies); a detached
`signature`/`publicKey` pair is a reserved field for signed feeds and is not yet verified. A feed's
trust SHALL be surfaced to
...
- **WHEN** the default feed loads, or a feed's declared integrity/signature verifies
