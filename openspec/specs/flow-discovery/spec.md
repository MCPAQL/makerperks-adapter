...
## Purpose
A model- and provider-agnostic, server-side toolkit a connected LLM agent drives to discover a
perk's application flow: a research brief, model-free schema/provenance/eligibility verification, an
overlay diff, a use-or-discover entry point, and a fidelity oracle over the spikes. The server is
the substrate (no web, no model, no provider SDK); the agent supplies the model + web and produces
a verified, portable Flow Document. Consumes `flow-health`'s `rediscover` signal; feeds the piece-D
proposal queue. (#47 piece C.)
## Requirements
