# operator-authorization Specification

## Purpose
The zero-trust trust boundary for a hosted, multi-user directory (#90): every authenticated user is
untrusted and may only **propose**; only the host/**operator** may accept a proposal into the
canonical, served set. Operator identity is host-configured — **A)** admin on a governing repo
(`OPERATOR_REPO`), read with the user's **own** token, or **B)** a static login allowlist
(`OPERATOR_LOGINS`); both may be set (OR'd); a single-user local deployment is implicitly an
operator; a hosted deployment configuring neither **fails safe** (no operators). Status is resolved
**once per session** at the OAuth callback and carried as a boolean only — **no credential is stored
at rest**, and the broader OAuth scope is requested only when option A is active. The whole model
serves one invariant: **the server never initiates a state-changing outbound call** — every mutation
is a principal's action under their own authority, and the only outbound calls are reads (so PRs stay
out of the server, #87). Gates `accept_flow` + `set_acceptance_mode` (the dial is the operator's
auto-accept pre-authorization); `propose_flow`, discovery, and all READ ops stay open.
