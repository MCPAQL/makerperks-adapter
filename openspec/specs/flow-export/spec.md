# flow-export Specification

## Purpose
The read-out half of the flows.json round-trip (epic #84): emit the **effective curated overlay** —
the loaded `flows.json` merged with the registry's accepted overlay, accepted entries winning — as a
portable, re-ingestible `flows.json` document, with a per-slug `base`/`accepted` source breakdown for
transparency. Exposed as the read-only `export_flows` operation (available on every deployment; the
read-only endpoint exports just the loaded overlay) and as the dev `scripts/export-flows.mjs`, making
the overlay externally co-maintainable via the existing inbound `FLOWS_URL`/file path. Reconciling the
accepted overlay back into the durable file (so the file is the single source of truth) is a separate
capability (#87).
