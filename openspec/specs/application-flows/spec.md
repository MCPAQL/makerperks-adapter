...
flow per program that overrides the baseline, marked `confidence: curated`. Every returned
flow SHALL carry an explicit `gaps` list naming what is not known and must still be
discovered, so a guess is never presented as a fact.
...
- **WHEN** a curated record exists for a program's slug
