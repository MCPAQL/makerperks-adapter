# flow-reconcile Specification

## Purpose
The last piece of the flows.json portability story (#87, epic #84): make operator-blessed flows a
durable artifact both deployments can serve, and define the operator-run path for contributing them
upstream — without the server ever acting on GitHub. An operator-gated `reconcile_flows` flushes the
registry's accepted overlay into a shared KV mirror; the read-only public endpoint reads that mirror
(cached per isolate) and serves the blessed flows with **no redeploy**, while the stateful endpoint's
Durable Object stays the always-live layer — reconcile is the deliberate publish, not an automatic
effect of accepting. The serving ops depend on a narrow `AcceptedOverlay` so a registry-less endpoint
can serve the mirror; precedence is unchanged (derived ⊕ flows.json ⊕ accepted). Upstream
contribution is operator-run and server-credential-free: the operator pulls the effective overlay
(`export_flows`), extracts the MIT-safe data subset (`export-flows.mjs --mit`), and opens one
deduplicated PR with their own `gh`. The server opens no PR and holds no GitHub write credential —
its only state-changing action is the internal mirror write, under the #90 invariant.
