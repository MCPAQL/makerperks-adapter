...
test("stdio: full CRUDE + execute tools, introspect lists 27 ops", async () => {
  const { tools, ops } = await viaStdio();
  assert.deepEqual(tools, LOCAL_TOOLS);
  assert.equal(ops.length, 27);
...
test("streamable http: full CRUDE + execute tools, introspect lists 27 ops", async () => {
